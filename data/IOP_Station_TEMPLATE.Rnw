...
if (!is.null(IOP$CTD$Time)) {
  ...
  p1 <- p1 + labs(x=expression(paste("Temperature", " (°C)")) , y="Depth (m)")
  #print(p1)
...
if (instrument$BB9 == 1) {
    ...
    waves.bb9 = IOP$BB9$wl
    n.wl = length(waves.bb9)
    bb.z.d = IOP$BB9$Depth[ix.down]
    bb.d = IOP$BB9$bb.offset[ix.down,]
    bbP.d = IOP$BB9$bbP.offset[ix.down,]
    bb.d[bb.d < minbb] = NA
    bb.d[bb.d > maxbb] = NA
    ...
    if (BB9.UPCAST) {
      ...
      bb.u = IOP$BB9$bb.offset[ix.up,]
      bbP.u = IOP$BB9$bbP.offset[ix.up,]
      bb.u[bb.u < minbb] = NA
...
if (instrument$ACS == 1) {
  ...
  ix.wl = rep(NA,9)
  if (is.null(IOP$BB9$wl)) wl.selected = c(412,443,488,510,555,590,620,676,700) else wl.selected = IOP$BB9$wl
  for (j in 1:9){
